...
// Also compiles two custom-layer variants (no part fx): `#define TW_FIXED_SEG 3` with no seg attribute (checked against warpPoint with segment 3), and a float per-vertex `seg` + `segD`.
...
async function main(){
	...
	const reference=(kind:Kind)=>[0,1].map(soft=>{const P:number[]=[],M:number[]=[];for(let i=0;i<N;i++){
		...
		warpPoint(ws,q,a,b,w,!!soft,q,kind==='fixed'?0:dist[i]);warpNormal(ws,m,a,b,w,m);P.push(...q);M.push(...m);
	}return [P,M];}).flat();
