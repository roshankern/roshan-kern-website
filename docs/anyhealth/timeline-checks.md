...
| Variable | Effect |
|---|---|
| `ANYHEALTH_PLAYWRIGHT=<dir>` | A directory holding `node_modules/playwright-core`, kept **outside** the repo (no new dependency): `mkdir -p <dir> && cd <dir> && npm i playwright-core`. The clipping group's "GLSL/TS parity at scale" check and `npx tsx scripts/anyhealth-timeline-glsl.ts` (headless shader compile + parity) run headless Chromium through it. |
| `ANYHEALTH_CHROME=<binary>` | The Chromium to launch; default: the newest cached `~/Library/Caches/ms-playwright/chromium-*` build. |
| `ANYHEALTH_SKIP_GPU=1` | Skip the GPU parity check explicitly when playwright-core is not available. Without it (and without `ANYHEALTH_PLAYWRIGHT`) that check fails, so a missing browser never passes silently. The runner prints a one-line hint whenever GPU parity was skipped. |
