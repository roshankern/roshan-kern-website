...
- **Acne marks shared with isotretinoin.** Both the acne and isotretinoin scripts own the same seeded lesions, scars and PIH with the same look: the acne layer draws them throughout, the isotretinoin layer only during its course and only while isotretinoin is isolated. Isolating either one shows the lesions clearing, and the two copies are never drawn together.
