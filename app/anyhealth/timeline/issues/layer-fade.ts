 ...
 * - an already transparent material (callus, clots, skin marks) just scales its opacity and writes depth while faded (fadeMaterial: state only, no recompile);
 * - an opaque material (fragments, caps, wisdom teeth) never flips `transparent`: its mesh swaps to the material's permanent ghost twin (fadeMesh), a transparent clone sharing its shader hooks, defines and uniforms, writing depth.
 * A faded layer writes depth because it draws in pass 0: the ghost pass (engine.ts: depth pre-pass, then colour at LessEqual) must treat it as a frontmost surface, or the ghosts behind it would paint over it (a pop at the onset).
...
/** For a transparent material: `own` opacity × ghostOpacity(ghost); depth writes on while faded, `own` depthWrite at ghost 0; true if anything changed. Never touches `transparent` (no recompile). */
export function fadeMaterial(m:T.Material,own:{opacity:number;depthWrite:boolean},ghost:number):boolean{
	const f=ghostOpacity(ghost),opacity=own.opacity*f,depthWrite=f<1||own.depthWrite;let changed=false;
...
/** The permanent ghost twin of an opaque layer material (created once): a clone with the same defines, onBeforeCompile and program cache key (so the same uniform objects), transparent, writing depth. Call it after the material's shader hooks are final. */
export function ghostTwin(m:T.Material):T.Material{
	let t=twins.get(m);if(t)return t;
	t=m.clone();t.defines={...m.defines};t.onBeforeCompile=m.onBeforeCompile;t.customProgramCacheKey=m.customProgramCacheKey;t.transparent=true;t.depthWrite=true;twins.set(m,t);return t;
}
