...
export function wisdomLayer(onset:string):CustomLayer{
	const teeth:{mesh:T.Mesh;rest:Float32Array;centre:T.Vector3;up:number;cap:number}[]=[];let ctxRef:LayerContext|null=null,last='',mat:T.Material|null=null;
	...
	return {
		init(ctx){
			ctxRef=ctx;const m=ctx.material({color:TOOTH_COLOR,segment:'head'});mat=m;
		...
		update(day:number,frame:LayerFrame){
			...
			const age=(toDays(onset)+day-toDays(BIRTH_DATE))/365.25,show=wisdomShown(onset,day)&&frame.systemVisible('skeletal'),fade=ghostOpacity(frame.ghost);
