...
import {APPROACH_MS,MAX_STEP_MS} from './types';
...
export function createClock(s:Schedule):Clock{
	...
	const advance=(nowMs:number)=>{
		if(!playing)return;const next=storyMs+Math.min(MAX_STEP_MS,Math.max(0,nowMs-lastNow));lastNow=nowMs;const i=nextHold(storyMs,next);
	...
	const clock:Clock={
		...
		seekMs(ms){playing=false;holding=null;storyMs=Math.max(0,Math.min(s.totalMs,Number.isFinite(ms)?ms:0));rearm();},
