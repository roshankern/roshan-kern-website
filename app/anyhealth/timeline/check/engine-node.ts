...
/** Build the engine and call ready(). `skip` leaves those part names without a picker (so a layer that needs them fails to init). */
export function nodeEngine(g:NodeAtlas,o:{skip?:string[];renderer?:Parameters<typeof createEngine>[0]['renderer']}={}):{engine:Engine;scene:T.Scene;bounds:T.Box3[];pickers:(T.Mesh|undefined)[]}{
	...
	engine.ready(pickers);return {engine,scene,bounds,pickers};
}
