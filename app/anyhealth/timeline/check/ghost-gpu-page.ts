 ...
 * O (non-focus, same mesh, half behind F) and S (non-focus, its own translucent skin-like material). Behind them a MeshBasic ground (not an engine material) covers the view, and the scene has a background colour (a background that cleared would wipe pass 0).
...
export interface GhostGpuIn {date:string;focus:string;names:{F:string;N:string;H:string;S:string;O:string}}
type Px=[number,number,number,number];type Key='F'|'N'|'H'|'S'|'gap'|'O'|'FO'|'NO';
...
export async function run(a:GhostGpuIn):Promise<GhostGpuOut>{
	...
	const parts:{key:'F'|'N'|'H'|'S'|'O';name:string;system:SystemId;c:[number,number];z?:number}[]=[
	...
	const body=new T.Mesh(geometry([0,1,2,4]),solid),skinMesh=new T.Mesh(geometry([3]),skin);body.frustumCulled=skinMesh.frustumCulled=false;scene.add(body,skinMesh);
	...
	const all=new T.Box3();bounds.slice(0,4).forEach(b=>all.union(b));const ctr=all.getCenter(new T.Vector3()),half=Math.max(all.max.x-all.min.x,all.max.y-all.min.y)/2*1.15;
