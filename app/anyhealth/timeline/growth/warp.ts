...
/** S × the inverse transpose of ∂R/∂p at p applied to n (not normalized). */
function axialNormal(ws:WarpState,x:number,y:number,z:number,nx:number,ny:number,nz:number,out:number[]){
	axialCurves(ws,y,cv);const g=cv[1],fp=cv[6],A=cv[8]*(fp-g)+cv[7]*(x-cv[2]),B=cv[9]*(fp-g)+cv[7]*(z-cv[3]),S=ws.axial[48];
	out[0]=S*nx/g;out[1]=S*(ny-(A*nx+B*nz)/g)/fp;out[2]=S*nz/g;
}
...
export function warpState(rig:Rig,body:Body):WarpState{
	const n=SEGMENTS.length,S=body.scale,ws:WarpState={restJoint:new Float32Array(n*3),axis:new Float32Array(n*3),newJoint:new Float32Array(n*3),alongScale:new Float32Array(n),boneScale:new Float32Array(n),softScale:new Float32Array(n),ground:0,axial:new Float64Array(AXIAL_VEC4*4)};
	...
	segs.forEach((s,i)=>{
		...
		if(p<AXIAL_SEGMENTS)axialPoint(ws,ws.restJoint[i*3],ws.restJoint[i*3+1],ws.restJoint[i*3+2],q);else segPoint(ws,p,ws.boneScale[p],ws.restJoint[i*3],ws.restJoint[i*3+1],ws.restJoint[i*3+2],q);ws.newJoint.set(q,i*3);
	});
	let lo=Infinity;
...
export function warpPoint(ws:WarpState,p:Vec3,segA:number,segB:number,wA:number,soft:boolean,out:Vec3,dBone=0):Vec3{
	...
	if(soft&&dBone>0){
		// Axial segments inflate radially from the remap's rest centre curve, (x − c0_x, 0, z − c0_z), by S(gs − g)(y): one field for trunk, neck and head, independent of their weights.
		const add=(i:number,w:number)=>{if(i<AXIAL_SEGMENTS){axialCurves(ws,y,cv);const rx=x-cv[2],rz=z-cv[3],m=w*(cv[10]-cv[1])*Math.min(1,dBone/Math.max(Math.sqrt(rx*rx+rz*rz),1e-9));ix+=m*rx;iz+=m*rz;return;}const k=i*3,ax=ws.axis[k],ay=ws.axis[k+1],az=ws.axis[k+2],dx=x-ws.restJoint[k],dy=y-ws.restJoint[k+1],dz=z-ws.restJoint[k+2],t=dx*ax+dy*ay+dz*az,rx=dx-t*ax,ry=dy-t*ay,rz=dz-t*az,m=w*(ws.softScale[i]-g[i])*Math.min(1,dBone/Math.max(Math.sqrt(rx*rx+ry*ry+rz*rz),1e-9));ix+=m*rx;iy+=m*ry;iz+=m*rz;};
		add(segA,wA);if(wA<1)add(segB,1-wA);
	}
	if(segA<AXIAL_SEGMENTS)axialPoint(ws,x,y,z,out);else segPoint(ws,segA,g[segA],x,y,z,out);
	if(wA<1){const ax=out[0],ay=out[1],az=out[2];if(segB<AXIAL_SEGMENTS)axialPoint(ws,x,y,z,out);else segPoint(ws,segB,g[segB],x,y,z,out);out[0]=wA*ax+(1-wA)*out[0];out[1]=wA*ay+(1-wA)*out[1];out[2]=wA*az+(1-wA)*out[2];}
...
export function warpNormal(ws:WarpState,p:Vec3,n:Vec3,segA:number,segB:number,wA:number,out:Vec3):Vec3{
	const g=ws.boneScale,x=n[0],y=n[1],z=n[2],m=[0,0,0];let rx=0,ry=0,rz=0;
	const add=(i:number,w:number)=>{if(i<AXIAL_SEGMENTS){axialNormal(ws,p[0],p[1],p[2],x,y,z,m);rx+=w*m[0];ry+=w*m[1];rz+=w*m[2];return;}const k=i*3,ax=ws.axis[k],ay=ws.axis[k+1],az=ws.axis[k+2],t=x*ax+y*ay+z*az,ia=1/ws.alongScale[i],ig=1/g[i];rx+=w*(ia*t*ax+ig*(x-t*ax));ry+=w*(ia*t*ay+ig*(y-t*ay));rz+=w*(ia*t*az+ig*(z-t*az));};
	add(segA,wA);if(wA<1)add(segB,1-wA);
